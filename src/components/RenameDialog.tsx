      ...
      <AlertDialogContent className="max-w-lg max-h-[85vh] overflow-y-auto bg-card border-border">
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            ...
            Réorganiser le dossier par BPM
          </AlertDialogTitle>
          {!isNative && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-sm">
              ...
              <span className="text-destructive">
                Le renommage natif nécessite l'app mobile. Exportez le projet vers GitHub, puis compilez avec Capacitor.
              </span>
            </div>
          )}
          <AlertDialogDescription className="text-muted-foreground">
            {analyzedFiles.length} fichier(s) analysé(s) prêt(s) au renommage
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        {step === 'preview' && (
          <div className="space-y-4">
            <div className="max-h-64 overflow-y-auto space-y-1 pr-1">
              {preview.map((p, i) => (
                <div key={i} className="flex items-center gap-2 py-1.5 px-2.5 rounded bg-secondary/30 text-xs">
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="ghost" size="sm" onClick={() => setStep('config')}>Retour</Button>
              <Button size="sm" onClick={handleRename} disabled={!isNative} className="bg-primary text-primary-foreground">
                <FolderSync className="h-4 w-4 mr-1.5" />
                Renommer {preview.length} fichier(s)
              </Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            {result.errors.length === 0 ? (
              <div className="flex flex-col items-center py-6 space-y-3">
                ...
                <p className="text-sm font-medium">{result.success} fichier(s) renommé(s) avec succès</p>
                <p className="text-xs text-muted-foreground">Un backup a été sauvegardé automatiquement</p>
              </div>
            ) : (
              <div className="space-y-3">
        ...
        {step === 'rollback' && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {backupLogs.length === 0 ? 'Aucun historique de renommage.' : `${backupLogs.length} opération(s) enregistrée(s)`}
            </p>
            <div className="max-h-48 overflow-y-auto space-y-2">
              {backupLogs.map((log, i) => (
                <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-secondary/50 border border-border/50">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleRollback(log)} disabled={isProcessing}>
